...
class Calc:
    ...
    def calcAero(self, xValsUpper, xValsLower, cpLowerVals, cpUpperVals, y1Vals, V1Vals, p1Vals, alpha, VInf, pInf, rho):
        ...
        cl_c = cl*(1-(size_factor/a)-((1+a)/(a)**(3/2))*t_f*size_factor-(((1+a)*(1+0.4*M**2))/a)*wake_factor*cdWake)   # true lift coeff
        cd_c = cdWake*(1-((3-0.6*M**2)/a**(3/2))*size_factor*t_f-(((1+a)*(1+0.4*M**2))/a)*wake_factor*cdWake)     # true drag Cd
        cm_c = cmc4*(1-((1+a)/(a)**(3/2))*t_f*size_factor-(((1+a)*(1+0.4*M**2))/a)*wake_factor*cdWake)+(size_factor/4*a)*cl
